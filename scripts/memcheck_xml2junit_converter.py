...
# If the option `skip_tests` is defined, an `error` element is replaced
# by a `skipped` element.
#
# USAGE
#
#   python memcheck_xml2junit_converter.py [WORKING_DIRECTORY]
#			           	   [REPORT_DIRECTORY]
#                                          [OPTIONS]
#
# Run the script by supplying a working directory containing one or several xml
# files and a report directory where the converted files are collected.
#
# WORKING_DIRECTORY
# This variable sets the folder path to where the script searches for xml files
# to convert. Subdirectories within the working directory are also included.
#
# REPORT_DIRECTORY
# This variable defines the output folder where the converted JUnit xml files
# are collected.
#
# OPTIONS
# * skip_tests: Error elements in a Valgrind Memcheck xml file is replaced by
#               a skipped message type in the converted JUnit xml file.
#
...
try:
  ...
  print "Missing required arguments: WORKING_DIRECTORY, REPORT_DIRECTORY"
  exit()
...
for subdir, dirs, files in os.walk(working_directory):
  ...
  for filename in files:
    if "xml" in filename:
      ...
      output_filename = os.path.join(report_directory, filename)
