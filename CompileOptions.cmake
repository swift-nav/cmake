...
function(swift_set_compile_options)
    set(argOption "WARNING" "NO_EXCEPTIONS" "EXCEPTIONS" "NO_RTTI" "RTTI" "PEDANTIC" "PEDANTIC_CXX" "PEDANTIC_C")
    set(argSingle "")
    ...
    if(x_PEDANTIC)
      set(x_PEDANTIC_C ON)
      set(x_PEDANTIC_CXX ON)
    endif()

    if(x_PEDANTIC_C)
      if(${c_enabled} GREATER -1)
        check_c_compiler_flag("-Werror -pedantic" HAVE_C_FLAG_PEDANTIC)
        if(NOT ${HAVE_C_FLAG_PEDANTIC})
          unset(x_PEDANTIC_C)
        endif()
      endif()
    endif()
    ...
    if(x_PEDANTIC_CXX)
      if(${cxx_enabled} GREATER -1)
        check_cxx_compiler_flag("-Werror -pedantic" HAVE_CXX_FLAG_PEDANTIC)
        if(NOT ${HAVE_CXX_FLAG_PEDANTIC})
          unset(x_PEDANTIC_CXX)
        endif()
      endif()
    endif()
    ...
    foreach(target ${targets})
      if(cxx_enabled)
        if(x_PEDANTIC_CXX)
          target_compile_options(${target} PRIVATE $<$<COMPILE_LANGUAGE:CXX>:-pedantic>)
        endif()
      ...
      if(c_enabled)
        if(x_PEDANTIC_C)
          target_compile_options(${target} PRIVATE $<$<COMPILE_LANGUAGE:C>:-pedantic>)
        endif()
      endif()
